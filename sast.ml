...
let rec string_of_sexpr (t, e) = 
  "(" ^ string_of_typ t ^ " : " ^ (match e with
      ...
      | SBinop (e1, op, e2) -> string_of_sexpr e1 ^ " " ^ string_of_bin_op op ^ " " ^ string_of_sexpr e2
      | SAsn (v, e) -> v ^ " = " ^ string_of_sexpr e
      | SAugAsn (v, op, e) -> v ^ " " ^ string_of_aug_op op ^ " " ^ string_of_sexpr e
...
and string_of_sexprs l = 
  ...
  else if List.length l > 1 then string_of_sexpr (List.hd l) ^ ", " ^ string_of_sexprs (List.tl l)
  else ""
...
let rec string_of_sstmt = function 
    ...
    | SFunction (fname, formals, return_type, block) -> "def " ^ fname ^ "(" ^ string_of_formals formals formals ^ ") -> " ^ string_of_typ return_type ^ ":" ^ string_of_sblock block
    | SReturn e -> "return " ^ string_of_sexpr e ^ "\n"
    | SIf (e, if_block, elif_blocks) -> "if " ^ string_of_sexpr e ^ ":" ^ string_of_sblock if_block ^ string_of_selif_blocks elif_blocks
    | SWhile (e, block) -> "while " ^ string_of_sexpr e ^ ":" ^ string_of_sblock block
    | SFor (v, e, b) -> "for " ^ v ^ " in " ^ string_of_sexpr e ^ ":" ^ string_of_sblock b
...
and string_of_sline (l: sstmt): string =
  string_of_sstmt l
